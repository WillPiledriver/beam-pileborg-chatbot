...
class Connection(Evented):
	...
	def whisper(self, user, msg):
		self.websocket.send("method", user, msg, method="whisper")
		
	def get_users(self):
		session = Session()
		response = session.get(
			self._build_addr("/api/v1/chats/{id}/users".format(id=self.channel)),
			headers={"X-CSRF-Token": self.csrf_token}
		)
		...
		for d in response: users[d["userName"]] = d["userRoles"]
		return users
